...
// Full self-tracked stats from D1 visitors table
async function getSelfTrackedStats(startDate, endDate) {
  const rows = await d1Rows(
    "SELECT * FROM visitors WHERE date>=? AND date<=?",
    [startDate, endDate],
  );
  const all = rows.map((row) => ({ fields: row }));
...
// Self-tracked visitor regions from D1 (구 단위)
